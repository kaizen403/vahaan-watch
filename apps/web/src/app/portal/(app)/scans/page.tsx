...
export default function ScansPage() {
  ...
  const [hitlistTarget, setHitlistTarget] = useState<Detection | null>(null);
    ...
    async (pageToLoad: number, filtersToUse: Filters, isRefresh = false) => {
      ...
      try {
        ...
        if (filtersToUse.dateFrom)
          params.set("from", new Date(filtersToUse.dateFrom).toISOString());
        if (filtersToUse.dateTo)
          params.set(
            "to",
            new Date(filtersToUse.dateTo + "T23:59:59").toISOString(),
          );
        if (filtersToUse.workstationId)
          params.set("workstationId", filtersToUse.workstationId);
  ...
  return (
    <>
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                ...
                {Math.min(page * pageSize, total)} of {total} scan
                {total !== 1 ? "s" : ""}
              </p>
              <div className="flex items-center gap-2">
                <Button
      ...
      <AddToHitlistModal
        open={!!hitlistTarget}
        plate={hitlistTarget?.plate ?? ""}
        vehicleMake={hitlistTarget?.make}
        vehicleModel={hitlistTarget?.model}
        vehicleColor={hitlistTarget?.color}
        vehicleCategory={hitlistTarget?.category}
        country={hitlistTarget?.country}
        onClose={() => setHitlistTarget(null)}
      />
    </>
  );
}
